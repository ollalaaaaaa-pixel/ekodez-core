...
type Metric = { platform: string; campaign: string; spend: string; leads: number; revenue: string; cpl: string | null; romi: string | null; cpl_reason: string | null; romi_reason: string | null }
...
export default function AdsPage({ onNotificationsRead }: { onNotificationsRead: () => void }) {
  ...
  return <Space direction="vertical" size="large" style={{ width: '100%' }}>
    ...
    <Tabs items={[
      { key: 'metrics', label: 'Метрики', children: <Table rowKey={(row) => `${row.platform}-${row.campaign}`} dataSource={metrics} pagination={false} columns={[{ title: 'Площадка', dataIndex: 'platform' }, { title: 'Кампания', dataIndex: 'campaign' }, { title: 'Расход', dataIndex: 'spend' }, { title: 'Лиды', dataIndex: 'leads' }, { title: 'CPL', dataIndex: 'cpl', render: (value, row: Metric) => metricValue(value, row.cpl_reason) }, { title: 'ROMI, %', dataIndex: 'romi', render: (value, row: Metric) => metricValue(value, row.romi_reason) }]} /> },
